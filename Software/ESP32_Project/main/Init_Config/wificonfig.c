...
static void softap_event_handler(void *arg, esp_event_base_t event_base,int32_t event_id, void *event_data)
{
    if (event_id == WIFI_EVENT_AP_STACONNECTED)
    {
        ...
        //发送配置事件,停止定时器
        xEventGroupSetBits(Event_Group, CONFIGWIFITIMEDEL);
    }
...
void wifi_init_softap(void)
{
    ...
    if (ret == ESP_OK)
    {
        ESP_LOGI(TAG, "GET_IP:" IPSTR "", IP2STR(&local_ip.ip));
    }
}
...
void wifi_init_sta(_Bool Datafrom)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        if(Datafrom)
        {
            wifi_config.sta.ssid[i] = recvwifiname[i];
            wifi_config.sta.password[i] = recvwifissid[i];
        }
        else
        {
            wifi_config.sta.ssid[i] = readwifiname[i];
            wifi_config.sta.password[i] = readwifissid[i];
        }
    }

...
static esp_err_t http_resp_webfs_dir_html(httpd_req_t *req, const char *dirpath)
{
    ...
    extern const unsigned char webfs_start[] asm("_binary_webfs_html_start");
    extern const unsigned char webfs_end[]   asm("_binary_webfs_html_end");
    const size_t webfs_size = (webfs_end - webfs_start);

    /* Add file upload form and script which on execution sends a POST request to /upload */
    httpd_resp_send_chunk(req, (const char *)webfs_start, webfs_size);
...
static esp_err_t http_resp_wifiset_dir_html(httpd_req_t *req, const char *dirpath)
{
...
static esp_err_t http_resp_home_dir_html(httpd_req_t *req, const char *dirpath)
{
    extern const unsigned char home_start[] asm("_binary_home_html_start");
    extern const unsigned char home_end[] asm("_binary_home_html_end");
    size_t size = (home_end - home_start);
    httpd_resp_send(req, (const char *)home_start, size);
    return ESP_OK;
}

static esp_err_t wifiset_get_handler(httpd_req_t *req)
{
    ...
    printf("return http_resp_wifiset_dir_html ok! filepath:%s \n", filepath);
    http_resp_wifiset_dir_html(req, filepath);
    return ESP_OK;
}
...
static esp_err_t home_get_handler(httpd_req_t *req)
{
    ...
    printf("return http_resp_home_dir_html ok! filepath:%s \n", filepath);
    return http_resp_home_dir_html(req, filepath);
}
...
static esp_err_t download_get_handler(httpd_req_t *req)
{
    ...
    if (filename[strlen(filename) - 1] == '/') {
        return http_resp_webfs_dir_html(req, filepath);
    }
...
esp_err_t start_wifi_config_server(const char *base_path)
{
    ...
    /* URI handler for getting uploaded files */
    httpd_uri_t wifiset = {
        .uri = "/wifiset", // Match all URIs of type /path/to/file
        .method = HTTP_GET,
        .handler = wifiset_get_handler,
        .user_ctx = server_data // Pass server data as context
    };
    httpd_register_uri_handler(server, &wifiset);

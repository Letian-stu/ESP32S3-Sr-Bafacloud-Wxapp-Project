...
void setup_image_screen(lv_ui *ui, uint32_t time, uint32_t delay)
{
    ...
    lv_obj_set_size(ui->img_list, 280, 200);
    lv_obj_align_to(ui->img_list, ui->label, LV_ALIGN_OUT_BOTTOM_MID, 0, 25);
    ...
    for (btn_cnt = 1; btn_cnt <= 10; btn_cnt++)
    {
        ...
        lv_list_add_btn(ui->img_list, LV_SYMBOL_IMAGE, buf);
    }
